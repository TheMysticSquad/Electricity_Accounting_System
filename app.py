...
# Database connection
DB_URL = (
    "postgresql://neondb_owner:npg_OqnZp0BDwSE1"
    "@ep-shy-poetry-abbw3imb-pooler.eu-west-2.aws.neon.tech/neondb"
    "?sslmode=require&options=endpoint%3Dep-shy-poetry-abbw3imb"
)
...
app.layout = html.Div(style={'backgroundColor': '#f0f8ff', 'padding': '20px'}, children=[
    html.H1("Electricity Distribution Network Dashboard", style={'textAlign': 'center'}),
    ...
    html.Button("Export Report", id='export-button', n_clicks=0),
...
def export_report(n_clicks, export_format, substation_id, feeder_id):
    if not n_clicks:
        return ""

    figures = update_charts(substation_id, feeder_id)
    titles = ["Feeder Pie", "Feeder Bar", "DTR Pie", "DTR Bar"]
    href, filename = generate_report(figures, titles, export_format)
